...
export default function RegisterStep4Screen() {
  ...
  const [policeClearanceDoc, setPoliceClearanceDoc] = useState<string | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (draft) {
        ...
        if (draft.licenseExpiry || draft.expiryDate || draft.licenseExpiryDate) {
          setExpiryDate(draft.licenseExpiry || draft.expiryDate || draft.licenseExpiryDate);
        }
        if (draft.licenseFrontUrl || draft.licenseFrontPhoto) {
          setFrontImage(draft.licenseFrontUrl || draft.licenseFrontPhoto);
        }
        if (draft.licenseBackUrl || draft.licenseBackPhoto) {
          setBackImage(draft.licenseBackUrl || draft.licenseBackPhoto);
        }
        if (draft.policeClearanceDoc) {
          setPoliceClearanceDoc(draft.policeClearanceDoc);
        }
      }
    })();
  }, []);

  const handlePickImage = async (side: 'front' | 'back' | 'police') => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]?.uri) {
        ...
        if (side === 'front') {
          ...
          setPoliceClearanceDoc(localUri);
          setUploadingPolice(true);
        }

        try {
          const folder = side === 'police' ? 'clearances' : 'licenses';
          const res = await uploadApi.uploadImage(localUri, folder);
          ...
          if (side === 'front') {
            ...
            setPoliceClearanceDoc(uploadedUrl);
            await saveRegistrationDraft({ policeClearanceDoc: uploadedUrl });
          }
        } catch (upErr) {
          console.warn('Image upload warning:', upErr);
        } finally {
          if (side === 'front') setUploadingFront(false);
          else if (side === 'back') setUploadingBack(false);
          else setUploadingPolice(false);
        }
      }
    } catch (err: any) {
